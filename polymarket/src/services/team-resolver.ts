...
// ─── Name cleaning (Polymarket-specific) ────────────────────
...
export function resolveTeamName(polymarketName: string): string {
  if (aliasMap[polymarketName]) return aliasMap[polymarketName];
  const cleaned = cleanPolymarketName(polymarketName);
  if (aliasMap[cleaned]) return aliasMap[cleaned];
  return cleaned;
}

// ─── UCL fixture cache (bulk loaded) ────────────────────────
...
// Leagues MSI2026 covers — only match against these
...
/**
 * Bulk-load all fixtures from the matches table for tracked leagues.
 * Called once at startup and on each discovery refresh.
 */
...
/**
 * Look up a fixture_id from the cached fixtures (all tracked leagues).
 * No DB query — pure in-memory match.
 * Returns null if no match found (= skip this market).
 */
export function lookupFixtureId(
  ...
  for (const f of cachedFixtures) {
    ...
    // Fuzzy normalized match (either direction)
    if (
